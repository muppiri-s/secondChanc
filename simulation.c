...
void printFrames(int no_of_frames,frameType frames[])
{
    ...
    {
        printf("%d ",frames[i].page_address);
    }
    printf("\n");
}

// Function to find the victim page to be replaced and replace it accordingly by your new page
//It returns the updated pointer of location from where the new search starts next time
int findAndReplace(int referenced_page, int no_of_frames, frameType frames[],int pointer)
{
    while(true)
    {
        if(frames[pointer].reference_bit == 0)
        {
            ...
            frames[pointer].page_address = referenced_page;
...
bool isNeedForUpdate(int referenced_page, int no_of_frames, frameType frames[])
{
    ...
    for(int i = 0 ; i < no_of_frames; i++)
    {
        if(frames[i].page_address == referenced_page)        //page already exists in frame
        {
...
int applySecondChanceAlgo(char *reference_string,long no_of_frames,frameType frames[])
{
    ...
    if(isNeedForUpdate( reference_string ,no_of_frames,frames))
    {
        //if yes, then find the page in frames array which need to be replaced and replace accordingly
        pointer = findAndReplace(reference_string,no_of_frames,frames,pointer);
...
int main()
{
    char file_name[100], line[50], page_address[10];
    long page_size = 4, memory_size = 64, no_of_frames, total_page_fault = 0;

    // Read trace file
    printf("\nTrace File: \n");
    scanf("C:\\Users\\sahit\\CLionProjects\\untitled2\\%c",file_name);
    FILE* trace_file = fopen("C:\\Users\\sahit\\CLionProjects\\untitled2\\trace1.txt", "r");
    ...
    for(int i=0; i<no_of_frames; i++)
    {
        frames[i].page_address= -1;
        frames[i].reference_bit = false;
    }

    while(fgets(line, sizeof(line), trace_file)){
        memcpy(page_address, line , 8);
        printf("\n");
        printf("%s", page_address);
        total_page_fault = applySecondChanceAlgo(page_address, no_of_frames,frames);
    }

    // print total number of page faults
    printf("\nPage Faults: %ld",total_page_fault);
    return 0;
}